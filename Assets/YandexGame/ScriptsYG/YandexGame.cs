...
namespace YG
{
    ...
    public partial class YandexGame : MonoBehaviour
    {
        ...
        void OnApplicationFocus(bool hasFocus) { 
            if(hasFocus == false) SaveIntermediateProgressWhenClosing();
        }

        void OnApplicationPause(bool pauseStatus) {
            if(pauseStatus) SaveIntermediateProgressWhenClosing();
        }

        public void SaveIntermediateProgressWhenClosing() {
            SessionData sessionData = AllServices.Container.Single<SessionData>();
            if(sessionData != null ) {
                if(sessionData.currentGameState == StateGame.Game) {
                    YandexGame.savesData.topScore = sessionData.topScore;
                    YandexGame.savesData.currentScore = sessionData.currentScore;
                    ...
                    Fruit[] fruitsInGlasse = sessionData.fruitsInGlasseParent.GetComponentsInChildren<Fruit>();
                    YandexGame.savesData.positionFruitX = new float[fruitsInGlasse.Length];
                    YandexGame.savesData.positionFruitY = new float[fruitsInGlasse.Length];
                    YandexGame.savesData.positionFruitZ = new float[fruitsInGlasse.Length];
                    YandexGame.savesData.rotationFruitX = new float[fruitsInGlasse.Length];
                    YandexGame.savesData.rotationFruitY = new float[fruitsInGlasse.Length];
                    YandexGame.savesData.rotationFruitZ = new float[fruitsInGlasse.Length];
                    YandexGame.savesData.typeFruit = new int[fruitsInGlasse.Length];

                    for (int i = 0; i < fruitsInGlasse.Length; i++) { 
                        YandexGame.savesData.typeFruit[i] = (int)fruitsInGlasse[i].FruitType;
                        YandexGame.savesData.positionFruitX[i] = fruitsInGlasse[i].transform.position.x;
                        YandexGame.savesData.positionFruitY[i] = fruitsInGlasse[i].transform.position.y;
                        YandexGame.savesData.positionFruitZ[i] = fruitsInGlasse[i].transform.position.z;
                        YandexGame.savesData.rotationFruitX[i] = fruitsInGlasse[i].transform.rotation.eulerAngles.x;
                        YandexGame.savesData.rotationFruitY[i] = fruitsInGlasse[i].transform.rotation.eulerAngles.y;
                        YandexGame.savesData.rotationFruitZ[i] = fruitsInGlasse[i].transform.rotation.eulerAngles.z;
                    }
                    YandexGame.SaveProgress();
                    YandexGame.NewLeaderboardScores("FruitMixLB", sessionData.topScore);
                } 
            } 
        }
    }
